// This version interpolates between generations for a smoother sound
// interpolated parameters are: rate, duration.
...
Voice nextPopulation[population.cap()];
...
nextGeneration(population, nextPopulation, fitnesses, lisa);
...
while (true) {
    ...
    for (int k; k < 500; k++)
    {
        ...
        {
            //rate
            population[i].rate + (nextPopulation[i].rate-population[i].rate)*c => float rate;
            lisa.rate(i, rate);
            
            //duration
            population[i].size + (nextPopulation[i].size-population[i].size)*c => dur size;
            lisa.loopEnd(i, size);
        }
        10::ms => now;
    }
